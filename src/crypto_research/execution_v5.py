...
def _limit_fill(order: ResearchOrder, bars: pd.DataFrame, passive_cross_bps: float) -> FillResult:
    ...
    for row in bars.itertuples(index=False):
        if order.side > 0:
            crossed = float(row.low) <= limit * (1.0 - buffer)
        else:
            crossed = float(row.high) >= limit * (1.0 + buffer)
        if crossed:
...
def simulate_order(
    ...
    *,
    ...
    if order.order_type == "MARKET":
        price = _adverse(float(first["open"]), order.side, slippage_bps)
        return FillResult(True, price, pd.Timestamp(first["timestamp"]), False, "MARKET_FILL")

    if order.order_type in {"LIMIT", "POST_ONLY"}:
        if order.limit_price is None:
            raise ValueError("limit order requires limit_price")
        if order.order_type == "POST_ONLY":
            marketable = (order.side > 0 and float(order.limit_price) >= float(first["open"])) or (
                order.side < 0 and float(order.limit_price) <= float(first["open"])
            )
            if marketable:
    ...
    if order.order_type == "TRIGGER_MARKET":
        ...
        for row in work.itertuples(index=False):
            if order.side > 0 and float(row.high) >= trigger:
                base = max(trigger, float(row.open))
                return FillResult(True, _adverse(base, 1, slippage_bps), pd.Timestamp(row.timestamp), False, "TRIGGER_FILL")
            if order.side < 0 and float(row.low) <= trigger:
                base = min(trigger, float(row.open))
                return FillResult(True, _adverse(base, -1, slippage_bps), pd.Timestamp(row.timestamp), False, "TRIGGER_FILL")
    ...
    for row in work.itertuples(index=False):
        if order.side < 0:
            ...
            if float(row.low) <= trigger:
                base = min(trigger, float(row.open))
                return FillResult(True, _adverse(base, -1, slippage_bps), pd.Timestamp(row.timestamp), False, "TRAILING_TRIGGER")
...
def choose_execution_mode(*, urgency: float, vol_state: str, alpha_half_life_minutes: int) -> str:
    if not 0.0 <= urgency <= 1.0:
        raise ValueError("urgency must be in [0, 1]")
    if vol_state not in {"low", "mid", "normal", "high"}:
        raise ValueError("unknown vol_state")
    if alpha_half_life_minutes <= 0:
        raise ValueError("alpha_half_life_minutes must be positive")
    if urgency >= 0.65 or vol_state == "high" or alpha_half_life_minutes <= 60:
        return "MARKET"
    if urgency <= 0.20 and vol_state == "low" and alpha_half_life_minutes >= 240:
        return "POST_ONLY"
    return "MARKET"
